...
Процедура ЗаполнитьФормуПоВрачамПациента()
	...
	Если НЕ ОперацииWS.ВыполнитьОперацию("ПолучитьВрачейПациента", 
		Новый Структура("snils, МассивВрачейВКлиенте, ДатаЗаписи, ЖелаемаяДата", 
	...
	Иначе
		...
		Если Результат.Свойство("ВрачиРасписания") Тогда
			МассивВрачей = Результат.ВрачиРасписания;
		КонецЕсли; 
		
		Запрос = Новый Запрос(ТекстЗапросаВрачей());
		Запрос.УстановитьПараметр("МассивВрачей", МассивВрачей);
		...
		Пока Выборка.Следующий() Цикл
			...
			ПолеФото.ШрифтЗаголовка = Новый Шрифт(,, 12,,Истина);
			//ПолеФото.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Центр;
			...
			СтрокаФото = ТаблицаФото.Добавить();
			СтрокаФото.Врач = Выборка.Ссылка;
			СтрокаФото.ИмяФото = "Фото" + Строка(Сч); 
...
Процедура ФотоНажатие(Элемент, СтандартнаяОбработка)
	...
	СтрокиФото = ТаблицаФото.НайтиСтроки(Новый Структура("ИмяФото", Элемент.Имя)); 
	Если СтрокиФото.Количество() > 0 Тогда
		
		ПараметрыОткрытия = Новый Структура("Врач", СтрокиФото[0].Врач); 
		ОткрытьФорму("Обработка.АРМ_ЗаписьКВрачу.Форма.ФормаЗаписиПоДате", ПараметрыОткрытия, ЭтотОбъект);
